...
public class AnimationManager : MonoBehaviour
{
    ...
    KinectWrapper.NuiSkeletonFrame[] MySkeletonFrameTable;
    int baseSkeletonID = 0;
    ...
    private List<AvatarAnimator> Player1Controllers;
    private List<AvatarAnimator> Player2Controllers;
    ...
    public Vector3 GetRawSkeletonJointPos(uint UserId, int joint)
    {
        ...
        if (UserId == Player1ID)
            if (joint >= 0 && joint < player1JointsPos.Length)
            {
                
                return (Vector3)MySkeletonFrameTable[baseSkeletonID].SkeletonData[player1Index].SkeletonPositions[joint];
        ...
        if (UserId == Player2ID)
        {
            if (joint >= 0 && joint < player2JointsPos.Length)
            {
                
                return (Vector3)MySkeletonFrameTable[baseSkeletonID].SkeletonData[player2Index].SkeletonPositions[joint];
        ...
        /*
         if (UserId == Player1ID)
             return joint >= 0 && joint < player1JointsPos.Length ? (Vector3)MySkeletonFrame.SkeletonData[player1Index].SkeletonPositions[joint] : Vector3.zero;
         else if (UserId == Player2ID)
             return joint >= 0 && joint < player2JointsPos.Length ? (Vector3)MySkeletonFrame.SkeletonData[player2Index].SkeletonPositions[joint] : Vector3.zero;
         */
        return Vector3.zero;
    }
    ...
    public void ResetAvatarControllers()
    {
        ...
        {
            AvatarAnimator[] avatars = FindObjectsOfType(typeof(AvatarAnimator)) as AvatarAnimator[];

            foreach (AvatarAnimator avatar in avatars)
            {
        ...
        if (Player1Controllers != null)
        {
            ...
            foreach (GameObject avatar in Player1Avatars)
            {
                ...
                {
                    AvatarAnimator controller = avatar.GetComponent<AvatarAnimator>();
    ...
    void Awake()
    {
        ...
        Player1Controllers = new List<AvatarAnimator>();
        Player2Controllers = new List<AvatarAnimator>();
        ...
        foreach (GameObject avatar in Player1Avatars)
        {
            if (avatar != null && avatar.activeInHierarchy)
            {
                Player1Controllers.Add(avatar.GetComponent<AvatarAnimator>());
            }
        }
        ...
        foreach (GameObject avatar in Player2Avatars)
        {
            if (avatar != null && avatar.activeInHierarchy)
            {
                Player2Controllers.Add(avatar.GetComponent<AvatarAnimator>());
            }
        }
        ...
        // Pull the AvatarAnimator from each of the players Avatars.
        Player1Controllers = new List<AvatarAnimator>();
        Player2Controllers = new List<AvatarAnimator>();
    ...
    void Update()
    {
        ...
        {
            foreach (AvatarAnimator controller in Player1Controllers)
            {
        ...
        {
            foreach (AvatarAnimator controller in Player2Controllers)
            {
                //if(controller.Active)
                {
    ...
    public void ProcessSkeleton(KinectWrapper.NuiSkeletonFrame[] MySkeletonFrameTable, int baseSkeletonID)
    {
        this.MySkeletonFrameTable = MySkeletonFrameTable;

        this.baseSkeletonID = baseSkeletonID;
...

        {
            ...
            skeletonData = MySkeletonFrameTable[baseSkeletonID].SkeletonData[0];
            ...
            if (skeletonData.eTrackingState == KinectWrapper.NuiSkeletonTrackingState.SkeletonTracked)
            {
                ...
                if (!AllPlayersCalibrated)
                {
                    ...
                    if (DetectClosestUser)
                    {
                        for (int j = 0; j < KinectWrapper.Constants.NuiSkeletonCount; j++)
                        {
                            if (j != 0)
                            {
                                KinectWrapper.NuiSkeletonData skeletonDataOther = MySkeletonFrameTable[baseSkeletonID].SkeletonData[j];
                    ...
                    if (bClosestUser)
                    {
                        CalibrateUser(userId, 0 + 1, ref skeletonData);
                    }
                }
                ...
                if (userId == Player2ID && Mathf.Abs(skeletonPos.z) >= MinUserDistance &&
                        (MaxUserDistance <= 0f || Mathf.Abs(skeletonPos.z) <= MaxUserDistance))
                {
                    player2Index = 0;
